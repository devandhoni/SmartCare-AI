...
function Today() {
    ...
    const dueCareActivities = data?.care_activities?.due ?? [];
    const overdueCareActivities = data?.care_activities?.overdue ?? [];
        ...
        <div className="w-full min-w-0 space-y-6">
            ...
            <section className="grid grid-cols-2 gap-3 lg:grid-cols-5">
                ...
                <SummaryCard
                    ...
                    value={
                        (summary.due_care_tasks ?? 0) +
                        (summary.overdue_care_tasks ?? 0)
                    }
                    description={
                        (summary.overdue_care_tasks ?? 0) > 0
                            ? `${summary.overdue_care_tasks} overdue`
            ...
            <section className="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)]">
                <div className="min-w-0 space-y-6">
                    ...
                    <Panel
                        ...
                        description="Scheduled routine care due today or already overdue."
                        action={
                            <button
                                type="button"
                    ...
                    >
                        {dueCareActivities.length === 0 &&
                        overdueCareActivities.length === 0 ? (
                            <EmptyState text="No scheduled routine care is currently due or overdue." />
                        ) : (
                            <div className="grid gap-4 lg:grid-cols-2">
                                <CareActivityGroup
                                    title="Overdue"
                                    items={overdueCareActivities}
                                    attention
                    ...
                    >
                        {homeLeave.length === 0 ? (
                            ...
                            <div className="space-y-3">
                                {homeLeave.map((leave) => (
                                    <div key={leave.id} className="rounded-xl border border-slate-200 p-4">
                                        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                            <div className="min-w-0">
                                                <button
                                                    type="button"
                                                    onClick={() => navigate(`/residents/${leave.resident_id}`)}
                                                    className="text-left font-semibold text-slate-900 hover:underline"
                                                >
                                                    {leave.resident}
                                                </button>
                                                <p className="mt-1 text-sm text-slate-500">
                                                    {leave.reason || "Home leave"}
                                                    {leave.destination ? ` • ${leave.destination}` : ""}
                                                </p>
                                                <p className="mt-2 text-xs text-slate-500">
                                                    Expected return: {formatDateTime(leave.expected_return_at)}
                                                </p>
                                            </div>
                                            <StatusBadge
                                                attention={leave.overdue}
                                                text={leave.overdue ? "Return Overdue" : "Away"}
                                            />
                                        </div>
                                    </div>
                                ))}
                        ...
                        {overdueHomeLeave.length > 0 && (
                            <p className="mt-4 text-sm font-semibold text-red-700">
                                ...
                                {overdueHomeLeave.length === 1 ? "" : "s"} overdue for return.
                            </p>
                        )}
                    </Panel>
                </div>
                ...
                <div className="min-w-0 space-y-6">
                                    ...
                                    >
                                        <div className="flex items-start justify-between gap-3">
                                            ...
                                            <StatusBadge
                                                ...
                                                text={task.overdue ? "Overdue" : task.priority || "Pending"}
                                            />
                                        </div>
                                        {task.scheduled_time && (
                                            <p className="mt-3 text-xs text-slate-400">
                                                Due {formatDateTime(task.scheduled_time)}
                                            </p>
                                        )}
                                    </button>
                                ))}
                    ...
                    <Panel title="Quick Actions" description="Open common daily care workflows.">
                        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1">
                            ...
                            <QuickAction title="Home Leave" description={`${homeLeave.length} currently away`} onClick={() => navigate("/home-leave")} />
                            <QuickAction title="Care Records" description="Open daily care records" onClick={() => navigate("/care-records")} />
                        </div>
                    </Panel>
                </div>
            </section>
...
function SummaryCard({ label, value, description, attention = false }) {
    return (
        <div className={`rounded-2xl border p-4 shadow-sm ${attention ? "border-amber-200 bg-amber-50" : "border-slate-200 bg-white"}`}>
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">{label}</p>
            <p className="mt-2 text-3xl font-bold text-slate-900">{value}</p>
            <p className={`mt-1 text-xs ${attention ? "font-medium text-amber-700" : "text-slate-400"}`}>{description}</p>
        </div>
    );
}
...
function CareActivityGroup({ title, items, attention = false, onOpenTasks, onResident }) {
    return (
        <div className={`rounded-xl border p-4 ${attention && items.length > 0 ? "border-red-200 bg-red-50" : "border-slate-200 bg-slate-50"}`}>
            <div className="flex items-center justify-between gap-3">
                <h3 className="font-semibold text-slate-900">{title}</h3>
                <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${attention && items.length > 0 ? "bg-red-100 text-red-700" : "bg-white text-slate-600"}`}>
                    {items.length}
                </span>
            </div>

            {items.length === 0 ? (
                <p className="mt-4 text-sm text-slate-500">No care activities in this group.</p>
            ) : (
                <div className="mt-4 space-y-2">
                    {items.slice(0, 5).map((item) => (
                        <div key={item.id} className="rounded-lg border border-slate-200 bg-white p-3">
                            <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                                <div className="min-w-0">
                                    <p className="text-sm font-semibold text-slate-900">{item.task_name}</p>
                                    <button
                                        type="button"
                                        onClick={() => onResident(item.resident_id)}
                                    ...
                                    {item.scheduled_time && (
                                        <p className="mt-2 text-xs text-slate-500">Scheduled {formatDateTime(item.scheduled_time)}</p>
                                    )}
                                </div>
                                <span className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-semibold ${item.overdue ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-800"}`}>
                                    {item.overdue ? "Overdue" : "Due"}
                                </span>
                            </div>
                        </div>
                    ))}
                    {items.length > 5 && <p className="pt-1 text-xs font-medium text-slate-500">+{items.length - 5} more</p>}
                </div>
            )}
...
function DueCheckCard({ title, count, items, buttonLabel, onOpen, onResident }) {
    return (
        <div className="rounded-xl border border-slate-200 p-4">
            <div className="flex items-center justify-between gap-3">
                <h3 className="font-semibold text-slate-900">{title}</h3>
                <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${count > 0 ? "bg-amber-100 text-amber-800" : "bg-emerald-100 text-emerald-700"}`}>
                    {count} due
                </span>
            </div>

            {items.length === 0 ? (
                <p className="mt-4 text-sm text-slate-500">All active residents are up to date.</p>
            ) : (
                <div className="mt-4 space-y-2">
                    {items.slice(0, 5).map((item) => (
                        <button
                    ...
                    {items.length > 5 && <p className="text-xs text-slate-400">+{items.length - 5} more residents</p>}
                </div>
            )}
...
function QuickAction({ title, description, onClick }) {
    return (
        <button type="button" onClick={onClick} className="rounded-xl border border-slate-200 p-4 text-left transition hover:border-slate-300 hover:bg-slate-50">
...
function StatusBadge({ text, attention = false }) {
    return (
        <span className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-semibold ${attention ? "bg-red-100 text-red-700" : "bg-slate-100 text-slate-600"}`}>
            {text}
        </span>
    );
}
...
function formatDashboardDate(value) {
    if (!value) return "Today's care overview";
